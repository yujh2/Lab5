...
public class Bank extends BankAccount{
    ...
    public boolean withdrawMoney(final BankAccount bankAccount, final double amount) {
        double balanceTemp = bankAccount.getAccountBalance();
        if (balanceTemp >= amount) {
            bankAccount.setAccountBalance(balanceTemp - amount);
            return true;
        } else {
            return false;
        }
    ...
    public boolean transferMoney(final BankAccount source, final BankAccount destination,
            final double amount) {
        double sourceBalTemp = source.getAccountBalance();
        double destinationBalTemp = destination.getAccountBalance();
        if (sourceBalTemp >= amount) {
            sourceBalTemp -= amount;
            destinationBalTemp += amount;
            source.setAccountBalance(sourceBalTemp);
            destination.setAccountBalance(destinationBalTemp);
            return true;
        } else {
            return false;
        }
